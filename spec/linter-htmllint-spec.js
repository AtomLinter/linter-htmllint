...
describe('The htmllint provider for Linter', () => {
  ...
  describe('The "Disable when no htmllint config is found" option', () => {
    it('lints files with no config when disabled', async () => {
      atom.config.set('linter-htmllint.disableWhenNoHtmllintConfig', false);
    ...
    it("doesn't lint files with no config when enabled", async () => {
      atom.config.set('linter-htmllint.disableWhenNoHtmllintConfig', true);
